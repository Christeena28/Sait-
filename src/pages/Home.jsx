    ...
    <div>
      ...
      <section className="section-hero" style={{ padding: '4.5rem 0 5rem' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '3.5rem', alignItems: 'center' }}>
            ...
            {/* Left Content */}
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255, 255, 255, 0.1)', padding: '0.35rem 0.85rem', borderRadius: '4px', marginBottom: '1.25rem' }}>
                <ShieldCheck size={16} style={{ color: 'var(--accent)' }} />
                <span style={{ fontSize: '0.8125rem', fontWeight: 600, letterSpacing: '0.06em', color: '#E8EFF7' }}>
                  DIVISION OF INFORMATION TECHNOLOGY · SOE, CUSAT
                </span>
              </div>

              <h1 style={{ color: '#FFFFFF', fontSize: 'clamp(2.2rem, 4vw, 3.2rem)', lineHeight: 1.18, marginBottom: '1.25rem', fontWeight: 700 }}>
                SAIT: where CUSAT's IT students learn, build and lead.
              </h1>

              <p style={{ color: '#C3CFDE', fontSize: '1.125rem', lineHeight: 1.65, marginBottom: '2rem', maxWidth: '580px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {/* SAIT Brand Badge */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'rgba(255, 255, 255, 0.06)', border: '1px solid rgba(255, 255, 255, 0.12)', padding: '1rem 1.25rem', borderRadius: '6px' }}>
                <img
                  src="/img/logo.png"
                  alt="SAIT Logo"
                ...
                />
                <div>
                  <h3 style={{ color: '#FFFFFF', fontSize: '1rem', margin: 0 }}>SAIT Association Portal</h3>
              ...
              {/* Next Event Card */}
              {nextEvent && (
                <div style={{ background: '#FFFFFF', borderRadius: '6px', padding: '1.25rem', border: '1px solid var(--border)', color: 'var(--text)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
                    <span className="badge badge-accent">Next Event</span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>{nextEvent.date}</span>
                  </div>
                  ...
                  <Link to="/events" style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--primary)', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
              ...
              {/* Latest Notice Card */}
              {latestNotice && (
                <div style={{ background: 'rgba(255, 255, 255, 0.08)', borderRadius: '6px', padding: '1.25rem', border: '1px solid rgba(255, 255, 255, 0.14)', color: '#FFFFFF' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <span style={{ background: 'rgba(224, 165, 38, 0.2)', color: 'var(--accent)', fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.5rem', borderRadius: '4px', textTransform: 'uppercase' }}>
                      Latest Notice
                    </span>
                    <span style={{ fontSize: '0.75rem', color: '#A9B8CC' }}>{latestNotice.date}</span>
                  </div>
                  <h4 style={{ fontSize: '0.975rem', color: '#FFFFFF', marginBottom: '0.4rem' }}>
                    {latestNotice.title}
                  </h4>
                  <Link to="/notices" style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--accent)', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
      ...
      <section className="section-alt" style={{ padding: '2.5rem 0' }}>
        <div className="container">
          <div className="stats-grid-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
            {STATS.map((stat, idx) => (
              <div key={idx} style={{ padding: '1rem', textAlign: 'center', borderRight: idx !== STATS.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <div style={{ fontFamily: 'var(--heading)', fontSize: '2.4rem', fontWeight: 700, color: 'var(--primary)', lineHeight: 1.1, marginBottom: '0.4rem' }}>
                  {stat.value}
                </div>
                <div style={{ fontSize: '0.9rem', color: 'var(--muted)', fontWeight: 500, marginBottom: '0.25rem' }}>
                  {stat.label}
                </div>
                {stat.sample && (
              ...
              {[
                ...
                return (
                  <div key={i} className="card" style={{ padding: '1.25rem' }}>
                    <IconComponent size={22} style={{ color: 'var(--secondary)', marginBottom: '0.65rem' }} />
                    <h3 style={{ fontSize: '1.05rem', marginBottom: '0.35rem', color: 'var(--primary-dark)' }}>{item.title}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.25rem' }}>
            {[
              ...
              return (
                ...
                >
                  <div style={{ background: '#EEF1F6', padding: '0.65rem', borderRadius: '6px', color: 'var(--primary)', marginBottom: '1rem' }}>
                    <Icon size={24} />
                  </div>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <h2 className="section-title">Industry Placements & Recruiting</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.75rem' }}>
                {PLACEMENT_STATS.stats.slice(0, 3).map((st, i) => (
                  <div key={i} className="card" style={{ padding: '1rem', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--primary)', fontFamily: 'var(--heading)' }}>
                      {st.value}
                    </div>
              ...
              <div style={{ marginBottom: '1.75rem' }}>
                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text)', marginBottom: '0.65rem' }}>
                  Prominent Campus Recruiters:
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {PLACEMENT_STATS.topRecruiters.slice(0, 8).map((rec, i) => (
                    <span key={i} style={{ background: '#FFFFFF', border: '1px solid var(--border)', padding: '0.25rem 0.65rem', borderRadius: '4px', fontSize: '0.825rem', color: 'var(--text)' }}>
                      {rec}
                    </span>
                  ))}
                  <span style={{ padding: '0.25rem 0.65rem', fontSize: '0.825rem', color: 'var(--secondary)', fontWeight: 500 }}>
                    + many more
                  </span>
                </div>
              </div>
            ...
            {/* Recruiter & Career Highlight Box */}
            <div className="card" style={{ padding: '2rem', backgroundColor: '#FFFFFF' }}>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1rem', padding: 0 }}>
                <li style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  <div>
                    <strong style={{ fontSize: '0.95rem' }}>Peer-Driven DSA & Mock Rounds:</strong>
                    <p style={{ fontSize: '0.85rem', color: 'var(--muted)', margin: 0 }}>Weekly coding practice and technical mock interview sessions hosted by placed seniors.</p>
                  </div>
                </li>
                <li style={{ display: 'flex', gap: '0.75rem' }}>
                  <CheckCircle2 size={20} style={{ color: 'var(--success)', flexShrink: 0, marginTop: '2px' }} />
                  <div>
                    <strong style={{ fontSize: '0.95rem' }}>Central Placement Coordination:</strong>
                    <p style={{ fontSize: '0.85rem', color: 'var(--muted)', margin: 0 }}>Direct integration with the Central Placement Office (CPO) of CUSAT.</p>
                  </div>
                </li>
                <li style={{ display: 'flex', gap: '0.75rem' }}>
                  <CheckCircle2 size={20} style={{ color: 'var(--success)', flexShrink: 0, marginTop: '2px' }} />
                  <div>
                    <strong style={{ fontSize: '0.95rem' }}>Alumni Referral Networks:</strong>
                    <p style={{ fontSize: '0.85rem', color: 'var(--muted)', margin: 0 }}>Exclusive referral opportunities and mentorship from IT alumni at top tech firms.</p>
                  </div>
                </li>
              </ul>
            </div>
      ...
      {/* 9. ACTIVITY LOGGER INTRODUCTION + LEADERBOARD */}
      <section className="section">
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '3rem', alignItems: 'center' }}>
            <div>
              <span className="section-eyebrow">Student Recognition Program</span>
              <h2 className="section-title">SAIT Activity Logger</h2>
              ...
              <p style={{ color: 'var(--text)', marginBottom: '1.75rem', fontSize: '0.95rem' }}>
            ...
            <div className="card" style={{ padding: '1.5rem', backgroundColor: '#FFFFFF' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.75rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <h3 style={{ fontSize: '1.1rem', margin: 0, color: 'var(--primary-dark)' }}>Activity Leaderboard</h3>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {[
                  ...
                  <div key={row.rank} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0.75rem', borderRadius: '4px', background: row.rank === 1 ? '#FFF8E7' : '#F7F6F2' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <span style={{ fontWeight: 700, color: row.rank === 1 ? 'var(--accent)' : 'var(--muted)', width: '18px', textAlign: 'center' }}>
                        #{row.rank}
                      </span>
                      <div>
                        <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text)' }}>{row.name}</div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{row.year}</div>
                      </div>
                    </div>
      ...
      {/* 11. CONTACT CTA */}
      <section style={{ backgroundColor: 'var(--primary)', color: '#FFFFFF', padding: '4rem 0' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: '720px' }}>
          <span style={{ color: 'var(--accent)', textTransform: 'uppercase', fontSize: '0.8rem', fontWeight: 700, letterSpacing: '0.08em', display: 'block', marginBottom: '0.5rem' }}>
            Connect With SAIT
          </span>
          <h2 style={{ color: '#FFFFFF', fontSize: '2.2rem', marginBottom: '1rem', fontWeight: 700 }}>
            Have an Idea, Workshop Proposal or Collaboration?
          </h2>
          <p style={{ color: '#C3CFDE', fontSize: '1.05rem', lineHeight: 1.6, marginBottom: '2rem' }}>
